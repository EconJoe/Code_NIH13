...
keep pmid treated `covariatepool'
...
* ESTIMATE THE 2ND ORDER TERMS
use finalvars, clear
local varnum=0
local covsvariates_log=""
local covsvariates_list=""
levelsof varname, local(levels)
foreach cov in `levels' {
	if (`varnum'==0) {
		local covsvariates_log="`cov'"
		local covsvariates_list="`cov'"
		local varnum=1
	}
	else {
		local covsvariates_log="`covsvariates_log'" + "+" + "`cov'"
		local covsvariates_list="`covsvariates_list'" + " " + "`cov'"
	}
}
display in red "`covsvariates_log'"
display in red "`covsvariates_list'"

use covariates_all, clear
keep pmid treated `covsvariates_list'
save covariates_all, replace

local varnum=1
use covariates_all, clear
foreach cov in `covsvariates_list' {
	rename `cov' _`varnum'
...
forvalues i=1/26 {
	forvalues j=1/26 {
		gen _`varnum' = _`i'*_`j'
		local varnum=`varnum'+1
		compress
	}
}

save covariates_all_2ndorder, replace
...
keep pmid treated `covsvariates_list'
cd D:\Research\Projects\NIHMandate\NIH14\Data
save covariates_all_large, replace
...
save covariates_all_large_2ndorder, replace
...
keep pmid treated `covsvariates_list'
cd D:\Research\Projects\NIHMandate\NIH14\Data
save covariates_matched_all, replace
...
save covariates_matched_all_2ndorder, replace
