...
clear
gen estimate = .
save coeffs_poisson_covs_fc_dev_2yr, replace
...
keep pmid fc_dev_2yr ta nih post yr_* nih_post ta_post nih_ta nih_ta_post `covariates' group_* year
tab group_country, gen(county_)
poi2hdfe fc_dev_2yr nih post ta nih_post ta_post nih_ta nih_ta_post yr_* county_* `covariates', cluster(group_ui4) id1(group_ui) id2(group_country)
local obs = e(N)
parmest, norestore
gen obs = `obs'
gen trimmed = "No"
gen sample = "PRCA 1-to-1"
append using coeffs_poisson_covs_fc_dev_2yr
save coeffs_poisson_covs_fc_dev_2yr, replace
