...
* Unfortunatly they reghdfe handles factor variables can be wonky, so we have to do this manually.
...
*******************************************************************************************************************
*******************************************************************************************************************
* Publication Patterns Variables (ta, scinat)

* Thi ssection computes the non-stratified (Regression) estimates
set more off
local depvars `" "ta" "scinat" "'
foreach depvar in `depvars' {

	local filename "coeffs_pubpatterns_reg_`depvar'"
	clear
	gen parm=""
	cd $outpath
	...
	foreach sample in `samples' {
		...
		foreach treatment in `treatments' {	
			...
			* DID;
			...
			cd $inpath; estimation "temp" "`depvar'" "group_ui4" "`sample'" "quad" "2013" "dd" "_t" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_ui4" "`sample'" "lin" "2013" "dd" "" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_ui4" "`sample'" "lin" "2013" "dd" "_t" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			#delimit cr
		}
	}
}

* This section computes the statified estimates.
set more off
local depvars `" "ta" "scinat" "'
foreach depvar in `depvars' {

	local filename "coeffs_pubpatterns_strat_`depvar'"
	clear
	gen parm=""
	cd $outpath
	save `filename', replace

	local samples `" "prca_1to1" "prca_full" "journal" "medline" "'ocal samples `" "prca_1to1" "'
	foreach sample in `samples' {

		local treatments `" "Standard" "Linear" "'
		foreach treatment in `treatments' {	

			#delimit ;
			* DID;
			cd $inpath; estimation "temp" "`depvar'" "group_ui4" "`sample'" "quad" "2013" "dd" "" "`treatment'" "Yes"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_ui4" "`sample'" "quad" "2013" "dd" "_t" "`treatment'" "Yes"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_ui4" "`sample'" "lin" "2013" "dd" "" "`treatment'" "Yes"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_ui4" "`sample'" "lin" "2013" "dd" "_t" "`treatment'" "Yes"; cd $outpath; append using `filename'; save `filename', replace;
			#delimit cr
		}
	}
}
...
foreach depvar in `depvars' {
	...
	foreach sample in `samples' {
		...
		foreach treatment in `treatments' {
			...
			#delimit ;
			* Triple Differences;
			cd $inpath; estimation "temp" "`depvar'" "group_nlmid" "`sample'" "quad" "2011" "ddd" "" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_nlmid" "`sample'" "quad" "2011" "ddd" "_t" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_nlmid" "`sample'" "lin" "2011" "ddd" "" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_nlmid" "`sample'" "lin" "2011" "ddd" "_t" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			
			* DID;
			cd $inpath; estimation "temp" "`depvar'" "group_nlmid" "`sample'" "quad" "2011" "dd" "" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_nlmid" "`sample'" "quad" "2011" "dd" "_t" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_nlmid" "`sample'" "lin" "2011" "dd" "" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			cd $inpath; estimation "temp" "`depvar'" "group_nlmid" "`sample'" "lin" "2011" "dd" "_t" "`treatment'" "No"; cd $outpath; append using `filename'; save `filename', replace;
			#delimit cr
		}
	}
}
...
cd $outpath
use coeffs_pubpatterns_reg_ta_prca_1to1, clear
append using coeffs_pubpatterns_reg_ta_prca_full
append using coeffs_pubpatterns_reg_ta_journal
append using coeffs_pubpatterns_reg_ta_medline

append using coeffs_pubpatterns_reg_scinat_prca_1to1
append using coeffs_pubpatterns_reg_scinat_prca_full
append using coeffs_pubpatterns_reg_scinat_journal
append using coeffs_pubpatterns_reg_scinat_medline

gen clustvar="group_ui4"
compress
save coeffs_pubpatterns_reg, replace




cd $outpath
use coeffs_pubpatterns_strat_ta_prca_1to1, clear
append using coeffs_pubpatterns_strat_ta_prca_full
append using coeffs_pubpatterns_strat_ta_journal
append using coeffs_pubpatterns_strat_ta_medline

append using coeffs_pubpatterns_strat_scinat_prca_1to1
append using coeffs_pubpatterns_strat_scinat_prca_full
append using coeffs_pubpatterns_strat_scinat_journal
append using coeffs_pubpatterns_strat_scinat_medline

replace endyear="2013" if missing(endyear)
gen clustvar="group_ui4"
compress
save coeffs_pubpatterns_strat, replace




cd $outpath
use coeffs_forwardcites_reg_fc_2yr_prca_1to1, clear
append using coeffs_forwardcites_reg_fc_2yr_prca_full
append using coeffs_forwardcites_reg_fc_2yr_journal
append using coeffs_forwardcites_reg_fc_2yr_medline

append using coeffs_forwardcites_reg_fc_com_2yr_prca_1to1
append using coeffs_forwardcites_reg_fc_com_2yr_prca_full
append using coeffs_forwardcites_reg_fc_com_2yr_journal
append using coeffs_forwardcites_reg_fc_com_2yr_medline

append using coeffs_forwardcites_reg_fc_dev_2yr_prca_1to1
append using coeffs_forwardcites_reg_fc_dev_2yr_prca_full
append using coeffs_forwardcites_reg_fc_dev_2yr_journal
append using coeffs_forwardcites_reg_fc_dev_2yr_medline

gen endyear="2011"
gen clustvar="group_nlmid"
compress
save coeffs_forwardcites_reg, replace
...
cd $outpath
use coeffs_forwardcites_strat_fc_2yr_prca_1to1, clear
append using coeffs_forwardcites_strat_fc_2yr_prca_full
append using coeffs_forwardcites_strat_fc_2yr_journal
append using coeffs_forwardcites_strat_fc_2yr_medline

append using coeffs_forwardcites_strat_fc_com_2yr_prca_1to1
append using coeffs_forwardcites_strat_fc_com_2yr_prca_full
append using coeffs_forwardcites_strat_fc_com_2yr_journal
append using coeffs_forwardcites_strat_fc_com_2yr_medline

append using coeffs_forwardcites_strat_fc_dev_2yr_prca_1to1
append using coeffs_forwardcites_strat_fc_dev_2yr_prca_full
append using coeffs_forwardcites_strat_fc_dev_2yr_journal
append using coeffs_forwardcites_strat_fc_dev_2yr_medline

gen endyear="2011"
gen clustvar="group_nlmid"
compress
save coeffs_forwardcites_strat, replace
